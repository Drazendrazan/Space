...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName(Lang:t("blip.office"))
            ...
            if PlayerJob.name == "realestate" then
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, Lang:t("text.store_vehicle"))
                                if IsControlJustPressed(0, 38) then
                                    DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, Lang:t("text.get_vehicle"))
                                if IsControlJustPressed(0, 38) then
                                    if IsControlJustPressed(0, 38) then
                                        EstateMenu()
                                    end
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, Lang:t("text.on_duty"))
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, Lang:t("text.off_duty"))
                        end
...
function EstateMenu()
    exports['qb-menu']:openMenu({
        {
            header = Lang:t("jobmenu.vehicles"),
            isMenuHeader = true
        },
        {
            header = Lang:t("jobmenu.take_out_vehicle"),
            txt = Lang:t("jobmenu.take_out_vehicle_help"),
            params = {
                event = "qb-realestatejob:client:VehicleList"
            }
        },
        {
            header = Lang:t("jobmenu.close"),
            txt = "",
            params = {
...
RegisterNetEvent("qb-realestatejob:client:VehicleList", function()
    local VehicleList = {
        {
            header = Lang:t("jobmenu.vehicle_list"),
            isMenuHeader = true
        },
    }
